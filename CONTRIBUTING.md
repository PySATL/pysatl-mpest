...
The guidelines are divided into sections depending on the part of the project you want to contribute.

## Rules for adding commits

Create a new branch, if you want to add something new.
Recommended naming branch is `<contributor-nickname>/<name-of-stuff>`
...
* `chore` for changes outside the code, for example, gitignore and readme updates

The `<scope>` field contains context of the changes.

The `<body>` field contains the gist of the changes in the present imperative in English without the dot in at the end,
the first word is a verb with a lowercase letter.

Examples:

* Good: "feat(dists): add uniform distribution"
* Bad: "Added uniform distribution."



More about conventional commits you can read [here](https://www.conventionalcommits.org/ru/v1.0.0/).
